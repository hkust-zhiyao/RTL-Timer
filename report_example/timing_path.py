...
class timing_path(object):
    ...
    def get_path_embedding(self):
        ...
        for cell in self.path[1:-1]: ### without start-end registers
            ...
            trans = self.node_dict[cell].trans

            trans = self.norm_slew(trans)

            cell_name = tpe + str(trans)
    ...
    def norm_fanout(self, fanout_num):
        ### 1-10, 15-105, 150-950, 1000 (29 numbers)
        if fanout_num <= 10:
            ret_num = fanout_num
    ...
    def norm_slew(self, slew_num):
        slew_num = slew_num*1000
        ### 1-10, 15-95, 150-950, 1500-9500, 10000 (29 numbers)
        if slew_num <= 10:
            ret_num = int(slew_num)
        elif 10 < slew_num < 100:
            ret_num = int(slew_num/10)*10+5
        elif 100 <= slew_num < 1000:
            ret_num = int(slew_num/100)*100+50
        elif 1000 <= slew_num < 10000:
            ret_num = int(slew_num/1000)*1000+500
        else:
            ret_num = 10000
...
class timing_slack_rpt_parser(object):
    def __init__(self):
        self.path_cnt = 0
        self.reg_lst = []
        self.reg_slack_dct = {}
    ...
    def parse_rpt(self, rpt_path):
        ...
        for idx, line in enumerate(lines):
            ...
            if s:
                ...
                self.reg_lst.append(copy.copy(ep))
                ...
                path_delay = feat_vec[0]
                self.reg_slack_dct[ep] = path_delay
                self.reg_feat_dct[ep] = feat_vec
                # print(f"ep: {ep}, delay: {path_delay}")

        self.path_cnt = len(self.reg_slack_dct)
        
        
        # return self.reg_slack_dct